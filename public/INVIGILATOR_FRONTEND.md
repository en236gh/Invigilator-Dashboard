...
Use `venueId = 16` when checking in those students. Using another assigned venue (e.g. Natural Science Hall `19`) marks `WRONG_VENUE`.
...
3. `GET /api/attendance/lookup?computerNumber=&examSessionId=`
4. Show photo, name, computer number, programme, allocated venue, seat
5. Invigilator verifies visually
6. `POST /api/attendance/check-in` with `{ computerNumber, examSessionId, venueId, verificationMethod: "COMPUTER" }`
7. Handle 409 duplicate: “already checked in”
